...
- Don't default every commit/PR title to `docs:` just because the diff is markdown, `references/pr-writing.md`'s "editing conventional commits" gives that lowest-common-denominator treatment to a genuine documentation repo, but here the markdown *is* the skill's behavior. Pick the type by what changed about that behavior: `feat:` for a new rule, section, or technique the skill didn't cover before; `fix:` for correcting a mistake in an existing rule (wrong guidance, a broken example, a rendering bug in documented syntax); `docs:` only for meta-documentation about the repo itself, this file, the README, the license, not for a change to `SKILL.md` or anything under `references/`
