...
- Hard cap: 5 lines per inline comment, and most fit in 1-2. A comment that's still running past 5 lines isn't documenting one non-obvious thing anymore, it's carrying an explanation the code should carry instead: split the logic into a well-named helper, shorten the comment, or move the detail into a docstring/README where longer explanation belongs
